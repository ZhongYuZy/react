...
import AddRotation from '@/components/user/add';
import EditRotation from '@/components/user/edit';
...
const User = () => {
  ...
  let addok = (e: any) => {
    ...
    dispatch({
      type: 'user/getTopics',
      payload: {
        avatar: e.url.file.response.data,
        username: e.username,
        email: e.email,
        mobile: e.mobile,
        password: e.password,
  ...
  let onChange = (text: any, record: any) => {
    ...
    dispatch({
      ...
      payload: { id: record._id, status: text },
    });
  };
  let columns: any = [
    //表格数据
